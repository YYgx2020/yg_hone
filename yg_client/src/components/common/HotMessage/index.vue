    ...
    <ul class="hot_messsage_ul">
      ...
      <li class="message_item" v-for="(item, index) in hotMessageList" :key="item.id">
        <div class="web_info_container">
          ...
          <div class="like_row">
            <span :style="!item.likeClick? '': 'color: #1e80ff;'" @click="dianzanEvent(index, item)"><i class="iconfont icon-31dianzan"></i>&nbsp;{{item.like}}</span>
          </div>
        </div>
      </li>
    </ul>
  ...
  methods: {
    ...
    dianzanEvent(index, item) {
      ...
      if (item.likeClick) {
        ...
        // this.$refs.dianzan.style.color = '#8a919f';
      } else {
        like++
        item.likeClick = true
        console.log(2);
        // this.$refs.dianzan.style.color = '#1e80ff';
      }
